...
$(function() {
	$("#btnWrite").click(function() {
		location.href = "${path}/webProject/write.jsp";
	});
});
function list(page) {
...
<table style="font-size:20px; border-style:none; margin-top: 15px; margin-left:-168px; width:1500px;" border="1">
	<tr class="menu_" style="text-decoration: none;">
	...
	<c:forEach var="dto" items="${list}">
		<c:choose>
			<c:when test="${dto.show == 'y'}">
				<tr class="menu_1" style="font-weight: bolder;">
					<td width="5%" align="center">${dto.num}</td>
					<td width="10%">${dto.writer}</td>
					<td>
						<c:forEach var="i" begin="1" end="${dto.re_level}">&nbsp;&nbsp;&nbsp;</c:forEach>
						<a style="text-decoration: none; color: black;" href="${path}/board_servlet/view.do?num=${dto.num}">${dto.subject}</a>
						<c:if test="${dto.comment_count > 0}">
							<span style="color:red; font-size: .8em;">(${dto.comment_count})</span>
						</c:if>
					</td>
			...
			<c:otherwise>
				<tr class="menu_1">
					...
					<td colspan="6" align="center" style="color:red; font-weight: bolder;">&nbsp;삭제된 게시물입니다.</td>
				</tr>
			</c:otherwise>
		</c:choose>
	</c:forEach>
	<tr class="menu_1">
		<td colspan="7" style="color: black; text-align: center;">
